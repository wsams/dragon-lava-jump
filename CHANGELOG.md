...
- **Onscreen control icons:** Replaced text labels under Left/Right/Down/Breath/Jump/Boost with inline SVG icons (arrows and flame) so long-press does not trigger OS text selection. Labels removed; buttons keep aria-labels for accessibility. Boost uses left–right outward arrows (← →).
- **Documentation:** README and .cursorrules updated with Instructions dialog (menu button, "How to play" overlay) and on-screen control icons (no text labels; icon meanings and implementation notes).
